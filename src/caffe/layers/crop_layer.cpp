...
#include "caffe/layers/vision_layers.hpp" // Martin Kersner, 2015/12/23
...
void CropLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int layer_ind = 0; layer_ind < this->net_->top_vecs().size(); ++layer_ind) {
  ...
  vector<DiagonalAffineMap<Dtype> > coord_maps(2, DiagonalAffineMap<Dtype>::identity(2));

  std::cout << "XXX: " << coord_maps[0].coefs()[0].first << std::endl << std::flush; // Martin Kersner, 2016/01/04
  std::cout << "XXX: " << coord_maps[0].coefs()[0].second << std::endl << std::flush; // Martin Kersner, 2016/01/04
  std::cout << "XXX: " << coord_maps[0].coefs()[1].first << std::endl << std::flush; // Martin Kersner, 2016/01/04
  std::cout << "XXX: " << coord_maps[0].coefs()[1].second << std::endl << std::flush; // Martin Kersner, 2016/01/04
  std::cout << "XXX: " << coord_maps[1].coefs()[0].first << std::endl << std::flush; // Martin Kersner, 2016/01/04
  std::cout << "XXX: " << coord_maps[1].coefs()[0].second << std::endl << std::flush; // Martin Kersner, 2016/01/04
  std::cout << "XXX: " << coord_maps[1].coefs()[1].first << std::endl << std::flush; // Martin Kersner, 2016/01/04
  std::cout << "XXX: " << coord_maps[1].coefs()[1].second << std::endl << std::flush; // Martin Kersner, 2016/01/04

  for (int i = 0; i < 2; ++i) {
    for (Blob<Dtype>* blob = bottom[i]; blob != inter_blob; blob = this->net_->bottom_vecs()[down_map[blob]][0]) {
      shared_ptr<Layer<Dtype> > layer = this->net_->layers()[down_map[blob]];
      std::cout << layer->type() << std::endl << std::flush; // Martin Kersner, 2016/01/04
  ...
  std::cout << "BEFORE" << std::endl << std::flush; // Martin Kersner, 2016/01/04
  DiagonalAffineMap<Dtype> crop_map = coord_maps[1].compose(coord_maps[0].inv());
  std::cout << "AFTER" << std::endl << std::flush; // Martin Kersner, 2016/01/04
  for (int i = 0; i < 2; ++i) {
    // Check for scale mismatch (unfortunately, CHECK_DOUBLE_EQ does not
    // support a message like the other CHECKs).
    std::cout << "CROP FIRST: " << crop_map.coefs()[i].first << std::endl << std::flush; // Martin Kersner, 2016/01/04
    std::cout << "CROP FIRST: " << crop_map.coefs()[1].first << std::endl << std::flush; // Martin Kersner, 2016/01/04
    std::cout << "CROP SECOND: " << crop_map.coefs()[i].second << std::endl << std::flush; // Martin Kersner, 2016/01/04
    std::cout << "CROP SECOND: " << crop_map.coefs()[1].second << std::endl << std::flush; // Martin Kersner, 2016/01/04
    ...
    CHECK_DOUBLE_EQ(crop_map.coefs()[i].second, round(crop_map.coefs()[i].second));
  }
...
REGISTER_LAYER_CLASS(Crop); // Martin Kersner, 2015/12/29
//REGISTER_LAYER_CLASS_LEGACY(CROP, CropLayer); // Martin Kersner, 2015/12/18
